...
function removeBinding(twinId: string): void {
  const bindings = readBindings();
  delete bindings[twinId];
  localStorage.setItem(BINDINGS_KEY, JSON.stringify(bindings));
}
...
export function ReservoirDigitalTwinPanel() {
  ...
  const activeDatasetId = inferDatasetForTwin(activeTwin, datasets.data ?? []);
  ...
  const create = useMutation({
    mutationFn: () =>
      createTwin({
        reservoir_id: twinId.trim(),
        name: twinName.trim(),
      }),
    onSuccess: async (created) => {
      writeBinding(created.reservoir_id, {
        datasetId,
        assetId,
        wellId,
        field,
      });
      setSelectedTwin(created.reservoir_id);
      setCreateOpen(false);
      await queryClient.invalidateQueries({ queryKey: ["twins"] });
    },
  });

  const remove = useMutation({
    mutationFn: () => deleteTwin(activeTwinId),
    onSuccess: async () => {
      removeBinding(activeTwinId);
      setSelectedTwin("");
      await queryClient.invalidateQueries({ queryKey: ["twins"] });
    },
  });
  ...
  function chooseDataset(nextDatasetId: string): void {
    setCreateValidationError("");
    setDatasetId(nextDatasetId);
    const dataset = datasets.data?.find((item) => item.dataset_id === nextDatasetId);
    const linkedAsset = assets.data?.find(
      (item) => item.dataset_id === nextDatasetId,
    );
    const nextWell =
      linkedAsset?.well || dataset?.well_name || dataset?.name || "WELL";
    const nextField = linkedAsset?.field || dataset?.field_name || "";
    setAssetId(linkedAsset?.asset_id ?? "");
    setWellId(nextWell);
    setField(nextField);
    setTwinId(`${slug(nextWell)}-TWIN`);
    setTwinName(`${nextWell} Digital Well Twin`);
  }

  function openCreateDialog(): void {
    setCreateValidationError("");
    if (!datasetId && (datasets.data?.length ?? 0) > 0) {
      chooseDataset(datasets.data![0].dataset_id);
    }
    setCreateOpen(true);
  }
  ...
  function submitCreate(event: FormEvent<HTMLFormElement>): void {
    ...
    if (twinId.trim().length < 2) missing.push("Twin ID");
    if (twinName.trim().length < 2) missing.push("Twin name");
    if (wellId.trim().length < 1) missing.push("Well");

    if (missing.length > 0) {
      setCreateValidationError(`Complete the following field${missing.length > 1 ? "s" : ""}: ${missing.join(", ")}.`);
      return;
    }
  ...
  return (
    <Stack spacing={3}>
        ...
        <Grid container spacing={2} alignItems="center">
          <Grid item xs={12} md={5}>
            <FormControl fullWidth>
              <InputLabel>Digital well twin</InputLabel>
              <Select
                label="Digital well twin"
                value={activeTwinId}
                onChange={(event) => setSelectedTwin(event.target.value)}
              >
                <MenuItem value="">No twin selected</MenuItem>
                {twinList.map((twin) => (
          ...
          <Grid item xs={12} md={4}>
            <Stack direction="row" gap={1}>
              ...
              <Button
                color="error"
                startIcon={<Trash2 />}
                disabled={!activeTwinId || remove.isPending}
      ...
      {!activeTwinId && !loading && (
        <Alert severity="info">
          Create a Digital Well Twin from a registered well-log dataset. The creation form works independently of browser focus and Developer Tools.
        </Alert>
      )}
        ...
        <>
          {!activeDatasetId && (
            <Alert severity="warning">
              This older twin has no stored dataset binding. Create a new well twin or use a twin whose name matches a registered well dataset.
            </Alert>
          )}
          ...
          {replay.isError && (
            <Alert severity="error">
              The linked dataset replay could not be loaded for this twin.
            </Alert>
          )}
            ...
            <Stack spacing={2}>
              <Paper variant="outlined" sx={{ p: 1 }}>
                {events.length > 0 ? (
                  ...
                  <Alert severity="info">No replay states are available for the linked dataset.</Alert>
                )}
              </Paper>
      ...
      >
        <Box component="form" onSubmit={submitCreate}>
          <DialogTitle>Create Digital Well Twin</DialogTitle>
          <DialogContent>
            <Stack spacing={2} mt={1}>
              <Alert severity="info">
                Select a well-log dataset for replay, or create the twin manually and link a dataset later. PetroEdge uses historical depth replay as the simulated live feed until field connectors are configured.
              </Alert>
                ...
                >
                  {(datasets.data ?? []).map((dataset) => (
                    <MenuItem key={dataset.dataset_id} value={dataset.dataset_id}>
                      {dataset.name}{dataset.well_name ? ` · ${dataset.well_name}` : ""}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(event) => {
                    ...
                    if (nextAsset) {
                      setWellId(nextAsset.well);
                      setField(nextAsset.field);
                      if (nextAsset.dataset_id) chooseDataset(nextAsset.dataset_id);
                    }
                  }}
                >
                  <MenuItem value="">Use dataset metadata</MenuItem>
                  {(assets.data ?? []).map((assetItem) => (
                    <MenuItem key={assetItem.asset_id} value={assetItem.asset_id}>
                      {assetItem.field} · {assetItem.well}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <TextField
                    ...
                    label="Twin ID"
                    value={twinId}
                    onChange={(event) => setTwinId(slug(event.target.value))}
                  />
                </Grid>
                <Grid item xs={12} md={6}>
                  <TextField
                    ...
                    label="Twin name"
                    value={twinName}
                    onChange={(event) => setTwinName(event.target.value)}
                  />
                </Grid>
                <Grid item xs={12} md={6}>
                  <TextField
              ...
              {createValidationError && (
                <Alert severity="warning">{createValidationError}</Alert>
              )}
              {create.isError && (
                <Alert severity="error">
                  {create.error instanceof Error
                    ? create.error.message
                    : "Digital Well Twin creation failed."}
                </Alert>
              )}
            </Stack>
          </DialogContent>
          <DialogActions>
            ...
            >
              {create.isPending ? "Creating…" : "Create persistent twin"}
            </Button>
          </DialogActions>
        </Box>
      </Dialog>
