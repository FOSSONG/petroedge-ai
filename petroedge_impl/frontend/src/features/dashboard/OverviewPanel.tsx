...
import { Activity, BriefcaseBusiness, CheckCircle2, CircleDashed, Database, RefreshCw, Server, Workflow } from "lucide-react";
...
export function OverviewPanel(){
 ...
 const selectedWell=sessionStorage.getItem("petroedge_selected_well_id");
 const selectedDataset=sessionStorage.getItem("petroedge_selected_dataset_id");
 ...
 return <Stack spacing={3}>
  <Paper className="feature-hero" sx={{p:{xs:3,md:4}}}>
   <Stack direction={{xs:"column",md:"row"}} justifyContent="space-between" gap={2} alignItems={{md:"center"}} sx={{position:"relative",zIndex:1}}>
    ...
    <Stack direction="row" spacing={1} flexWrap="wrap"><Chip label={realtime.connected?"Realtime connected":"Realtime offline"} color={realtime.connected?"success":"warning"}/><Button variant="contained" color="inherit" startIcon={<BriefcaseBusiness size={17}/>} onClick={()=>navigateTo("Assets")}>Assets</Button><Button variant="contained" color="inherit" startIcon={<Database size={17}/>} onClick={()=>navigateTo("Datasets")}>Datasets</Button><Button variant="contained" color="inherit" startIcon={<RefreshCw size={17}/>} onClick={()=>void refreshPlatform(qc,[["sprint2","operations"],["platform","datasets"],["wells"],["jobs"]])}>Refresh</Button></Stack>
   </Stack>
  </Paper>
  ...
  {(selectedWell||selectedDataset)&&<Alert severity="info">Operation context: {selectedWell?`well ${selectedWell}`:"all wells"}{selectedDataset?` · dataset ${selectedDataset}`:""}. Selections made in Assets and Datasets are available to operational workspaces.</Alert>}
