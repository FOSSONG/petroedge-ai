...
router = APIRouter(prefix="/v1", tags=["Reservoir Intelligence V1"])
...
def _interpret(dataset_id: str) -> dict[str, Any]:
    ...
    active_rows=[]
    for item in samples:
        if item["pay_flag"] == "Pay":
            active_rows.append(item)
        elif active_rows:
            intervals.append(active_rows)
            active_rows=[]
    if active_rows:
        intervals.append(active_rows)

    interpreted_intervals=[]
    for interval_rows in intervals:
        interval_probs={name:sum(float(row["fluid_probabilities"][name]) for row in interval_rows)/len(interval_rows) for name in ("oil","gas","water","residual_hydrocarbon")}
        interval_fluid=max(interval_probs,key=interval_probs.get)
        interpreted_intervals.append({
            "top_depth":interval_rows[0]["depth"],
            "base_depth":interval_rows[-1]["depth"],
            "gross_thickness":round(abs(float(interval_rows[-1]["depth"])-float(interval_rows[0]["depth"])),3),
            "samples":len(interval_rows),
            "primary_fluid":interval_fluid,
            "fluid_confidence":round(interval_probs[interval_fluid],4),
            "fluid_probabilities":{key:round(value,4) for key,value in interval_probs.items()},
            "mean_porosity":round(sum(float(row["porosity"]) for row in interval_rows)/len(interval_rows),4),
            "mean_water_saturation":round(sum(float(row["water_saturation"]) for row in interval_rows)/len(interval_rows),4),
            "mean_hydrocarbon_probability":round(sum(float(row["hydrocarbon_probability"]) for row in interval_rows)/len(interval_rows),4),
        })

    pay_rows=[row for row in samples if row["pay_flag"] == "Pay"]
    probability_rows=pay_rows or samples
    mean_fluid_probs={name:sum(float(row["fluid_probabilities"][name]) for row in probability_rows)/len(probability_rows) for name in ("oil","gas","water","residual_hydrocarbon")}
    primary_fluid=max(mean_fluid_probs,key=mean_fluid_probs.get)
    mapped_canonicals={value for value in mapping.values() if value}
    available_evidence={
        "resistivity":"resistivity_ohmm" in frame.columns,
        "density":"density_gcc" in frame.columns,
        "neutron":"neutron_porosity_vv" in frame.columns,
        "sonic":"sonic_usft" in frame.columns,
        "production_or_pvt":False,
    }
    evidence_count=sum(1 for key,value in available_evidence.items() if value and key != "production_or_pvt")
    reliability="high" if evidence_count>=4 else "moderate" if evidence_count>=3 else "low"
    summary={"rows_interpreted":len(samples),"mapped_curves":sum(1 for v in mapping.values() if v),"mean_porosity":round(float(porosity.mean()),4),"mean_water_saturation":round(float(sw.mean()),4),"mean_hydrocarbon_probability":round(float(hc.mean()),4),"pay_samples":int(pay.sum()),"pay_intervals":len(interpreted_intervals),"primary_fluid":primary_fluid,"fluid_probabilities":{k:round(v,4) for k,v in mean_fluid_probs.items()},"fluid_confidence":round(mean_fluid_probs[primary_fluid],4),"fluid_method":"density-neutron-sonic-resistivity evidence-weighted screening","fluid_basis":"pay intervals" if pay_rows else "whole logged interval","fluid_reliability":reliability,"fluid_evidence":available_evidence,"calibration":"Niger Delta screening calibration"}
    return {"dataset":dataset.model_dump(),"curve_mapping":mapping,"summary":summary,"intervals":interpreted_intervals,"samples":samples}
    ...
    if any(term in q for term in ("pay interval", "pay zone", "show pay", "depths of pay", "all pay")):
        ...
        if intervals:
            ...
            for idx, interval in enumerate(intervals, 1):
                ...
                parts.append(f"Pay {idx}: {top:.2f}–{base:.2f} m, gross thickness {abs(base-top):.2f} m")
        ...
        answer = f"The highest mapped resistivity occurs at {float(target['depth']):.2f} m: {float(target['rt']):.2f} ohm·m. The corresponding interpretation is {target['lithology']}, porosity {pct(target['porosity'])}, Sw {pct(target['water_saturation'])}."
        ...
        answer = f"The screening identified {len(anomalies)} anomalous sample(s), based on upper-tail resistivity or borehole enlargement. The most relevant anomaly is at {float(target['depth']):.2f} m with RT {float(target['rt']):.2f} ohm·m and caliper {float(target['caliper']):.2f} in."
        ...
        if matched:
            ...
            if valid:
                ...
                unit = "%" if key in {"porosity","water_saturation","shale_volume","hydrocarbon_probability","fluid_confidence"} else {"permeability_md":" mD","gr":" API","rt":" ohm·m","rhob":" g/cc","nphi":" v/v","dt":" µs/ft","depth":" m"}.get(key,"")
...
def log_signature_pdf(dataset_id: str, _: dict[str, Any] = Depends(READ_ROLES)):
    ...
    curve_specs = [("gr","GR (API)",False),("rt","RT (ohm·m)",True),("rhob","RHOB (g/cc)",False),("nphi","NPHI (v/v)",False),("porosity","Porosity (v/v)",False),("water_saturation","Sw (v/v)",False),("hydrocarbon_probability","HC probability",False)]
    ...
    styles=getSampleStyleSheet();story=[Paragraph("PetroEdge AI — Interactive Log Signature Report",styles["Title"]),Spacer(1,8)]
