...
class WellRepository(Repository[Well]):
    ...
    @staticmethod
    def _authorize(record):
        if record is not None:
            require_owner((record.metadata_json or {}).get("access_owner_id"))
        return record

    def get(self, record_id: str):
        return self._authorize(super().get(record_id))

    def get_by_well_id(self, well_id: str) -> Well | None:
        return self._authorize(self.session.scalar(select(Well).where(Well.well_id == well_id.strip())))

    def list_ordered(self, *, offset: int = 0, limit: int = 100) -> Sequence[Well]:
        records = self.session.scalars(select(Well).order_by(Well.created_at.desc())).all()
        return [r for r in records if visible((r.metadata_json or {}).get("access_owner_id"))][offset:offset + limit]

    def list(self, *, offset: int = 0, limit: int = 100):
        return self.list_ordered(offset=offset, limit=limit)

    def delete(self, record: Well, *, commit: bool = True):
        self._authorize(record)
        return super().delete(record, commit=commit)
        ...
        *,
        ...
        return self.add(
            Well(
                ...
                status=status,
                metadata_json={"access_owner_id": owner_id()},
            )
        )
