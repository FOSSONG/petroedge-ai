...
class ReportSourceLoader:
    def load(
        ...
        if source_type == "saved_analysis":
            from app.services.analysis_records import get
            record = get(source_id)
            values = {k: v for k, v in record.items() if isinstance(v, (str, int, float, bool))}
            return pd.DataFrame([{**record.get("input", {}), **values}]), {"saved_analysis": record, "well_id": record["provenance"]["well_id"]}
    ...
    def _load_dataset(
        ...
        if dataset_id.startswith("ds-"):
            ...
            get_dataset(dataset_id)  # Enforce ownership before reading the source file.
...
def _load_related_alerts(
    ...
    for alert in alerts:
        if not visible(alert.get("owner_id")):
            continue
        matches_source = (
...
class ReportService:
    ...
    def generate(
        ...
        if request.source_type == "saved_analysis":
            combined_metadata = {**dict(request.metadata or {}), **source_metadata}
            well_id = source_metadata["well_id"]
        ...
        alerts = (
            ...
            if request.include_alerts and request.source_type != "saved_analysis"
            else []
        )

        if request.source_type == "saved_analysis":
            from app.services.analysis_evidence import METRICS, LIMITATION
            record = source_metadata["saved_analysis"]
            analysis = ReportAnalysis(
                executive_summary={"analysis_id": request.source_id, "scope": "single saved sample", "limitation": LIMITATION, "provenance": record["provenance"]},
                data_quality={"scope": "stored result; no new QC or inference performed"},
                petrophysical_summary={k: {"value": record[k], "method": method} for k, method in METRICS.items() if k in record},
                reservoir_intervals=[], alert_summary={"scope": "no alerts inferred from this sample"},
                model_summary=record.get("explanation", {}),
    ...
    def list_reports(self) -> list[dict[str, Any]]:
        ...
        for directory in REPORT_ROOT.iterdir():
            ...
            try:
                ...
                if visible(payload.get("owner_id")):
    ...
    def get_report(self, report_id: str) -> dict[str, Any]:
        ...
        payload = _read_json(metadata_path)
        require_owner(payload.get("owner_id"))
        return payload
    ...
    def delete_report(self, report_id: str) -> bool:
        ...
        if directory.parent != REPORT_ROOT.resolve():
            raise ValueError("Report directory is outside report storage.")
        import shutil
