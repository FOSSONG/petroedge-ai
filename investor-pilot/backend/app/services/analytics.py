...
def analyze_sample(
    ...
    try:
        ...
        fallback_details["hydrocarbon_provenance"] = hydrocarbon_output.provenance
    ...
    try:
        ...
        fallback_details["lithology_provenance"] = lithology_output.provenance
        lithology_prediction = str(
            lithology_output.predictions[0]
        )

        # An undocumented numeric class is not a geological lithology label.
        try:
            float(lithology_prediction)
        except ValueError:
            pass
        else:
            fallback_details["lithology_raw_class"] = lithology_prediction
            fallback_details["lithology_label_status"] = "unmapped; verified class dictionary required"
            lithology_prediction = f"Unmapped class {lithology_prediction}"
    ...
    try:
        if not fallback_details.get("hydrocarbon_model_id"):
            raise ValueError("No classifier result available for explanation")
        explanation = explain(
            __import__("pandas").DataFrame(
                [feature_record]
            ), model_id=fallback_details["hydrocarbon_model_id"]
        )
    except Exception as exc:
        explanation = {
            "status": "unavailable",
    ...
    explanation = {
        "methodology": {"hydrocarbon_model_weight": 0.55, "hydrocarbon_heuristic_weight": 0.45, "porosity_saturation_permeability": "heuristic estimates, not trained regression outputs"},
        **explanation,
        **fallback_details,
    }
