...
ALLOWED_EXTENSIONS = {".csv", ".parquet", ".las", ".xlsx", ".xls", ".txt", ".asc", ".ascii", ".dat", ".tsv", ".dlis", ".pdf"}
DATASET_TYPES = {"technical_document", "well_log", "processed_well_log", "fluid_data", "pvt_data", "formation_water", "fluid_contacts", "pressure_data", "temperature_data", "core_data", "production_data", "training_dataset", "prediction_output"}
...
def _frame(path: Path, limit: int | None = None):
    ...
    from app.services.industry_files import TEXT_EXTENSIONS, read_text_table, read_las, read_dlis
    if suffix in TEXT_EXTENSIONS: return read_text_table(path, limit)
    if suffix==".dlis": return read_dlis(path, limit)
    if suffix==".pdf":
        from app.services.document_evidence import read_pdf
...
def _profile(path: Path):
    ...
    units: dict[str,str]=frame.attrs.get("source_units", {})
    metadata: dict[str,Any]=frame.attrs.get("source_metadata", {})
...
def register_upload(name: str, description: str | None, filename: str, stream: BinaryIO, owner_id: str | None, *, dataset_type: str="well_log", field_name: str|None=None, well_name: str|None=None, reservoir_name: str|None=None) -> DatasetSummary:
    owner_id = request_owner(owner_id)
    suffix=Path(filename).suffix.lower()
    if suffix not in ALLOWED_EXTENSIONS: raise ValueError("Unsupported table type. Use LAS, single-frame DLIS, headered ASCII/TXT/TSV/CSV, Parquet or Excel. Searchable PDFs are supported as technical documents; seismic and images require specialist interpretation.")
...
def list_datasets() -> list[DatasetSummary]:
    ...
    return [_row_to_model(row) for row in rows if visible(row["owner_id"])]
...
def get_dataset_path(dataset_id: str) -> Path:
    with connection() as conn: row=conn.execute("SELECT file_path,owner_id FROM datasets WHERE dataset_id=?",(dataset_id,)).fetchone()
    if row is None: raise KeyError(dataset_id)
    require_owner(row["owner_id"])
...
def preview_dataset(dataset_id: str, offset: int = 0, limit: int = 100):
    import pandas as pd
    ...
    frame = _frame(path, limit=offset + limit).iloc[offset:offset + limit].copy()
...
def lineage(dataset_id: str) -> DatasetLineageNode:
    ...
    allowed={item.dataset_id for item in list_datasets()}
    return DatasetLineageNode(dataset=dataset,parents=[p for p in parents if p in allowed],children=[c for c in children if c in allowed],operations=[op for op in ops if op["parent_dataset_id"] in allowed and op["child_dataset_id"] in allowed])
...
def _register_derived_frame(parent_id: str, frame, name: str, operation: dict[str, Any], actor_id: str | None):
    ...
    if operation.get("operation") == "explicit_curve_mapping":
        from app.preprocessing.curve_aliases import CANONICAL_UNITS
        inherited_units = {key: CANONICAL_UNITS[key] for key in operation["mapping"]["curves"]}
    if operation.get("operation") == "petroleum_preprocessing":
        renamed = operation.get("pipeline_summary", {}).get("curve_mapping", {})
        inherited_units = {renamed.get(key, key): value for key, value in inherited_units.items()}
        config = operation.get("configuration", {})
        if config.get("scaling", "none") != "none":
            for column in config.get("scaling_columns") or columns:
                inherited_units[column] = "scaled"
    elif operation.get("operation") == "spreadsheet_edit":
        for op in operation.get("operations", []):
            if op.get("action") == "rename_column" and op.get("column") in inherited_units:
                inherited_units[op["new_name"]] = inherited_units.pop(op["column"])
    ...
    with connection() as conn:
        ...
        conn.execute("INSERT INTO dataset_lineage(parent_dataset_id,child_dataset_id,operation,parameters_json,created_at) VALUES (?,?,?,?,?)",(parent_id,dataset_id,operation.get("operation","derived"),json.dumps(operation),now))
...
def prepare_dataset(dataset_id: str, request, actor_id: str | None):
    ...
    depth = request.depth_column or next((c for c in frame.columns if canonical_name_for(str(c)) == "depth_m"), "depth_m")
    if request.interpolate_limit or request.despike or request.smooth:
        if depth not in frame:
            raise ValueError("Select a depth column before interpolation, despiking or smoothing.")
        measured = pd.to_numeric(frame[depth], errors="coerce")
        if measured.isna().any() or measured.duplicated().any() or not measured.is_monotonic_increasing:
            raise ValueError("Review missing, duplicate or unordered depths before interpolation, despiking or smoothing.")
    well_columns = [c for c in frame if str(c).lower().replace(" ", "_") in {"well", "well_id", "well_name", "uwi"}]
    if (request.interpolate_limit or request.despike or request.smooth) and any(frame[c].nunique(dropna=False) > 1 for c in well_columns):
        raise ValueError("Select a single well before interpolation, despiking or smoothing; cross-well transformations are blocked.")
    result = run_preprocessing_pipeline(frame, config=PipelineConfig(depth_column=canonical_name_for(depth) or depth, interpolation_limit=request.interpolate_limit, interpolate_short_gaps=request.interpolate_limit > 0, despike=request.despike, smooth=request.smooth, clip_to_physical_ranges=request.clip_physical_ranges, add_default_caliper=False, replace_standard_nulls=False, sort_depth=False, remove_missing_depth=False, remove_duplicate_depths=False))
    ...
    name=request.name or f"{get_dataset(dataset_id).name} Â· ML-ready"
...
def edit_dataset(dataset_id: str, request, actor_id: str | None):
    ...
    name=request.name or f"{get_dataset(dataset_id).name} Â· edited"
