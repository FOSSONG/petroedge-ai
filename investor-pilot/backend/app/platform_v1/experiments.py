...
def create_experiment(payload: ExperimentCreate, owner_id: str | None) -> ExperimentSummary:
    ...
    if payload.dataset_id: get_dataset_path(payload.dataset_id)
    ...
    _EXECUTOR.submit(copy_context().run,_train,experiment_id,payload)
...
def list_experiments() -> list[ExperimentSummary]:
    ...
    return [_row_to_model(r) for r in rows if visible(r["owner_id"])]
