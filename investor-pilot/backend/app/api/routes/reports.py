...
from app.core.rbac import effective_roles
from app.core.security import get_current_user
from app.core.ownership import require_owner, set_principal
...
async def report_access(request: Request, user: dict[str, Any] = Depends(get_current_user)):
    roles=effective_roles(set(user.get("roles", [])))
    required={"viewer"} if request.method in {"GET","HEAD"} else {"admin","geoscientist","engineer"}
...
router = APIRouter(dependencies=[Depends(report_access)])
    ...
    def validate_source_type(
        ...
        if normalised not in allowed:
            raise ValueError(
                ...
                "dataset, prediction, stream, saved_analysis"
            )
...
def _report_directory(
    ...
    directory = (REPORT_ROOT / report_id).resolve()
...
def _resolve_report_file(
    ...
    _load_report_metadata(report_id)
    format_name = (
    ...
    if not path.resolve().is_relative_to(directory.resolve()) or directory.resolve().parent != REPORT_ROOT.resolve():
