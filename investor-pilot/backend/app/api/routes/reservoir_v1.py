...
from app.preprocessing.curve_aliases import canonical_name_for, curve_match, map_curve_columns, missing_ai_curves, header_parts, normalise_unit
...
SCREENING_RANGES = {
    "depth_m": (0, 20000), "gamma_ray_api": (0, 250),
    "resistivity_ohmm": (0.01, 10000), "density_gcc": (1.5, 3.2),
    "neutron_porosity_vv": (-0.15, 0.7), "sonic_usft": (35, 250),
    "caliper_in": (2, 30),
}
REQUIRED_SCREENING = {"depth_m", "gamma_ray_api", "resistivity_ohmm", "density_gcc", "neutron_porosity_vv"}
UNIT_FACTORS = {
    "depth_m": {"M": 1, "METRES": 1, "METERS": 1, "FT": .3048, "F": .3048},
    "gamma_ray_api": {"API": 1, "GAPI": 1},
    "resistivity_ohmm": {"OHMM": 1, "OHM.M": 1, "OHM-M": 1, "OHM*M": 1},
    "density_gcc": {"G/C3": 1, "G/CC": 1, "G/CM3": 1, "G/CM^3": 1, "KG/M3": .001},
    "neutron_porosity_vv": {"V/V": 1, "V/VOL": 1, "DEC": 1, "FRAC": 1, "%": .01, "PU": .01},
    "sonic_usft": {"US/FT": 1, "US/F": 1, "US/M": .3048},
    "caliper_in": {"IN": 1, "INCH": 1, "INCHES": 1, "CM": 1/2.54, "MM": 1/25.4},
}


def _frame(dataset_id: str, limit: int = 100000) -> tuple[pd.DataFrame, dict[str, str | None]]:
    from app.platform_v1.datasets import _frame as read_frame
    path = get_dataset_path(dataset_id)
    dataset = get_dataset(dataset_id)
    frame = read_frame(path, limit=limit + 1)
    if len(frame) > limit:
        raise ValueError(f"Interactive screening supports up to {limit:,} rows. Create a single-well depth subset first.")
    ...
    for target in SCREENING_RANGES:
        if target not in mapped:
            continue
        sources = [c for c in original if mapping[c] == target]
        if len(sources) != 1:
            if target in REQUIRED_SCREENING: problems.append(f"Select one source for {target}: {', '.join(sources)}")
            else: mapped[target] = float("nan")
            continue
        source = sources[0]
        unit = normalise_unit(dataset.units.get(source) or header_parts(source)[1])
        factor = UNIT_FACTORS[target].get(unit)
        # Explicit canonical column names encode units. Raw aliases do not.
        if not unit and source == target and not dataset.processing:
            factor = 1.0
        if factor is None:
            if target in REQUIRED_SCREENING: problems.append(f"{source}: declare supported source units before screening (found {unit or 'missing'}).")
            else: mapped[target] = float("nan")
        else:
            mapped[target] *= factor
        if target == "depth_m" and ("TVD" in source.upper() or "VERTICAL" in source.upper()):
            problems.append("Screening requires measured depth; vertical depth needs an explicit trajectory conversion.")
    well_columns = [c for c in frame if str(c).lower().replace(" ", "_") in {"well", "well_id", "well_name", "uwi"}]
    if any(frame[c].nunique(dropna=False) > 1 for c in well_columns):
        problems.append("Select one well before reservoir screening.")
    mapped.attrs["blocking_issues"] = problems
    return mapped, mapping


def _series(frame: pd.DataFrame, name: str, default: float) -> pd.Series:
    # Kept signature for internal callers; missing inputs are never fabricated.
    if name not in frame:
        raise ValueError(f"Required curve is missing: {name}")
...
def _interpret(dataset_id: str) -> dict[str, Any]:
    ...
    problems = frame.attrs.get("blocking_issues", [])
    missing = [c for c in REQUIRED_SCREENING if c not in frame]
    if missing or problems:
        raise ValueError("Screening blocked. " + "; ".join([*(f"Missing curve: {c}" for c in missing), *problems]))
    source_rows = len(frame)
    frame["source_row"] = range(source_rows)
    valid = pd.Series(True, index=frame.index)
    invalid_by_curve = {}
    for curve, (low, high) in SCREENING_RANGES.items():
        if curve not in frame: frame[curve] = float("nan")
        values = pd.to_numeric(frame[curve], errors="coerce")
        good = values.between(low, high) & ~values.isin([-999.25, -999, -9999, -99999])
        invalid_by_curve[curve] = int((~good).sum())
        if curve in REQUIRED_SCREENING: valid &= good
        else: frame[curve] = values.where(good)
    frame = frame.loc[valid].copy()
    if frame.empty:
        raise ValueError("No complete valid rows remain after QC. Invalid row counts by curve: " + str(invalid_by_curve) + ". Check measured depth (not Well_id), null markers and source units in LAS Wizard. Measured curves can be viewed separately in Interactive Logs.")
    depth = _series(frame, "depth_m", 0.0)
    if depth.duplicated().any() or not depth.is_monotonic_increasing:
        raise ValueError("Measured depth must be unique and increasing. Review duplicates and acquisition order before screening.")
    ...
    gas_score = ((0.60 * gas_effect + 0.20 * sonic_gas.fillna(0) + 0.20 * hc) / (0.8 + 0.2 * dt.notna())).clip(0, 1)
    ...
    for i in range(len(frame)):
        samples.append({"source_row":int(frame["source_row"].iloc[i]),"depth":round(float(depth.iloc[i]),3),"gr":round(float(gr.iloc[i]),3),"rt":round(float(rt.iloc[i]),4),"rhob":round(float(rhob.iloc[i]),4),"nphi":round(float(nphi.iloc[i]),4),"dt":round(float(dt.iloc[i]),3) if pd.notna(dt.iloc[i]) else None,"caliper":round(float(cali.iloc[i]),3) if pd.notna(cali.iloc[i]) else None,"porosity":round(float(porosity.iloc[i]),4),"water_saturation":round(float(sw.iloc[i]),4),"permeability_md":round(float(perm.iloc[i]),3),"shale_volume":round(float(vsh.iloc[i]),4),"hydrocarbon_probability":round(float(hc.iloc[i]),4),"fluid_type":str(fluid_labels.iloc[i]),"fluid_confidence":round(float(max(gas_probability.iloc[i],oil_probability.iloc[i],water_probability.iloc[i],residual_probability.iloc[i])),4),"fluid_probabilities":{"oil":round(float(oil_probability.iloc[i]),4),"gas":round(float(gas_probability.iloc[i]),4),"water":round(float(water_probability.iloc[i]),4),"residual_hydrocarbon":round(float(residual_probability.iloc[i]),4)},"lithology":str(lith.iloc[i]),"pay_flag":"Pay" if bool(pay.iloc[i]) else "Non-pay"})
    intervals=[]
    active=None
    previous = None
    typical_step = float(depth.diff().median()) if len(depth) > 1 else 0
    for item in samples:
        gap = previous is not None and (item["source_row"] != previous["source_row"] + 1 or (typical_step > 0 and item["depth"] - previous["depth"] > 3 * typical_step))
    ...
    summary={"rows_interpreted":len(samples),"mapped_curves":sum(1 for v in mapping.values() if v),"mean_porosity":round(float(porosity.mean()),4),"mean_water_saturation":round(float(sw.mean()),4),"mean_hydrocarbon_probability":round(float(hc.mean()),4),"pay_samples":int(pay.sum()),"pay_intervals":len(intervals),"primary_fluid":primary_fluid,"fluid_probabilities":{k:round(v,4) for k,v in mean_fluid_probs.items()},"fluid_confidence":round(mean_fluid_probs[primary_fluid],4),"fluid_method":"density-neutron-resistivity screening; sonic only where measured","missing_optional_curves":[c for c in ("sonic_usft","caliper_in") if frame[c].isna().all()],"calibration":"Uncalibrated empirical screening; no regional validation","output_kind":"heuristic_scores_not_probabilities","source_rows":source_rows,"excluded_rows":source_rows-len(frame),"invalid_by_curve":invalid_by_curve,"assumptions":"Matrix density 2.65 g/cm3; fluid density 1.0 g/cm3; Archie a=1, m=2, n=2, Rw=0.1 ohm.m; no temperature or formation calibration"}
...
@router.get("/datasets/{dataset_id}/crossplot-data")
def crossplot_data(dataset_id: str, _: dict[str, Any] = Depends(READ_ROLES)):
    """Full-depth measured values, shared aliases/units and pairwise QC.

    No interpretation or model inputs are manufactured here.
    """
    import json
    try:
        frame, mapping = _frame(dataset_id)
        if frame.attrs.get("blocking_issues"):
            raise ValueError("Resolve the curve/unit choices in LAS Wizard: " + "; ".join(frame.attrs["blocking_issues"]))
        source_rows = len(frame)
        output = pd.DataFrame(index=frame.index)
        for curve in SCREENING_RANGES:
            values = pd.to_numeric(frame[curve], errors="coerce") if curve in frame else pd.Series(float("nan"),index=frame.index)
            low, high = SCREENING_RANGES[curve]
            output[curve] = values.where(values.between(low, high))
        output["source_row"] = range(source_rows)
        return {"dataset_id":dataset_id, "source_rows":source_rows, "mapping":mapping,
                "rows":json.loads(output.to_json(orient="records")),
                "units":{"depth_m":"m","gamma_ray_api":"API","resistivity_ohmm":"ohm.m","density_gcc":"g/cm3","neutron_porosity_vv":"v/v"},
                "qc":"Invalid values remain missing. Each crossplot uses only its valid measured pairs."}
...
def las_wizard(dataset_id: str, _: dict[str, Any] = Depends(READ_ROLES)) -> dict[str, Any]:
    try:
        ...
        return {"dataset_id":dataset_id,"detected_curves":[str(c) for c in frame.columns],"mnemonic_mapping":mapping,"match_details":{c:curve_match(c) for c in mapping},"source_units":{c:get_dataset(dataset_id).units.get(c) or header_parts(c)[1] for c in mapping},"blocking_issues":frame.attrs.get("blocking_issues",[]),"qc":{"row_count":len(frame),"missing_cells":int(frame.isna().sum().sum()),"duplicate_rows":int(frame.duplicated().sum()),"ready":False,"curve_coverage_complete":not missing},"missing_recommended_curves":missing,"steps":["File read","Headers normalised","Curve aliases mapped","QC preview available","Review units, depth reference and missing values"]}
    except KeyError as exc: raise HTTPException(status_code=404, detail="Dataset not found.") from exc
    except (ValueError, OSError) as exc: raise HTTPException(status_code=422, detail=str(exc)) from exc


from app.services.well_curve_mapping import CurveMapping, UNITS, validate_mapping


@router.post("/datasets/{dataset_id}/mapped-copy", status_code=201)
def mapped_copy(dataset_id: str, mapping: CurveMapping,
                user: dict[str, Any] = Depends(require_roles("admin", "engineer", "geoscientist", "petrophysicist"))):
    ...
    try:
        ...
        if datasets._checksum(path) != dataset.checksum_sha256:
            raise ValueError("Source checksum changed; register the changed source as a new dataset.")
        frame = datasets._frame(path, limit=100001)
        if len(frame) > 100000: raise ValueError("Create a depth subset with at most 100,000 rows first.")
        validate_mapping(mapping, frame.columns)
        depth_source = mapping.curves["depth_m"].source.upper()
        if mapping.depth_reference != "MD" or "TVD" in depth_source or "VERTICAL" in depth_source:
            raise ValueError("Reservoir screening requires measured depth. Keep vertical depths in their original dataset.")
        output = pd.DataFrame(index=frame.index)
        for target, choice in mapping.curves.items():
            values = pd.to_numeric(frame[choice.source], errors="coerce")
            values = values.mask(values.isin(choice.null_values))
            output[target] = values * UNITS[target][choice.unit]
        depth = output["depth_m"]
        if canonical_name_for(mapping.curves["depth_m"].source) == "well_id" or not depth.between(0, 20000).any():
            raise ValueError("The selected depth column contains no usable measured depths. Select the numeric depth curve, not a well identifier (for example BOREHOLE-DEPTH.M).")
        # Preserve well identifiers to prevent accidental cross-well processing.
        for column in frame:
            if str(column).lower().replace(" ", "_") in {"well", "well_id", "well_name", "uwi"}:
                output[column] = frame[column]
        if datasets._checksum(path) != dataset.checksum_sha256:
            raise ValueError("Source changed while mapping; retry with a stable version.")
        return datasets._register_derived_frame(dataset_id, output, dataset.name + " - mapped units",
            {"operation":"explicit_curve_mapping", "mapping":mapping.model_dump(), "source_sha256":dataset.checksum_sha256}, user.get("user_id"))
    except KeyError as exc: raise HTTPException(404,"Dataset not found.") from exc
    except (ValueError,OSError) as exc: raise HTTPException(422,str(exc)) from exc
...
def replay(
    ...
    for index,row in enumerate(rows):
        ...
        if row["caliper"] is not None and row["caliper"]>10.5: alerts.append("Possible borehole enlargement")
...
def assistant(dataset_id: str, payload: AssistantRequest, _: dict[str, Any] = Depends(READ_ROLES)) -> dict[str, Any]:
    ...
    if any(term in q for term in ("pay interval", "pay zone", "show pay", "depths of pay", "all pay")):
        ...
        answer = f"The highest interpreted porosity occurs at {float(target['depth']):.2f} m: {pct(target['porosity'])}. The sample is {target['lithology']} with Sw {pct(target['water_saturation'])} and hydrocarbon screening score {pct(target['hydrocarbon_probability'])}."
    elif "lowest water" in q or "lowest sw" in q or "minimum sw" in q:
        intent = "lowest_water_saturation"; target = lowest_sw
        answer = f"The lowest interpreted water saturation occurs at {float(target['depth']):.2f} m: {pct(target['water_saturation'])}, with porosity {pct(target['porosity'])} and hydrocarbon screening score {pct(target['hydrocarbon_probability'])}."
        ...
        anomalies = [row for row in samples if float(row.get("rt", 0)) >= threshold or float(row.get("caliper") or 0) > 10.5]
        target = max(anomalies or samples, key=lambda row: float(row.get("hydrocarbon_probability", 0)))
        answer = f"The screening identified {len(anomalies)} anomalous sample(s), based on upper-tail resistivity or borehole enlargement. The most relevant anomaly is at {float(target['depth']):.2f} m with RT {float(target['rt']):.2f} ohmÂ·m and caliper {float(target['caliper'] or 0):.2f} in."
        ...
        answer = (f"The current log-based screening classifies {dataset_name} predominantly as {primary.title()} with {pct(confidence)} screening score. "
                  f"Normalised screening scores: {distribution}. This is an evidence-weighted screening result; formation testing, PVT, mud-log shows or production data are required for definitive fluid confirmation.")
        target = best_hc
    elif "compare" in q:
    ...
    evidence = [
        ...
        f"Calibration: {summary.get('calibration', 'Uncalibrated screening')}",
...
def log_signature_pdf(dataset_id: str, _: dict[str, Any] = Depends(READ_ROLES)):
    ...
    for index, row in enumerate(samples):
        if index and row["source_row"] != samples[index - 1]["source_row"] + 1:
            plot_samples.append({"depth": row["depth"]})
        plot_samples.append(row)
    depth = [float(r["depth"]) for r in plot_samples]
    curve_specs = [("gr","GR (API)",False),("rt","RT (ohm.m)",True),("rhob","RHOB (g/cm3)",False),("nphi","NPHI (v/v)",False),("porosity","Estimated porosity (v/v)",False),("water_saturation","Estimated Sw (v/v)",False),("hydrocarbon_probability","HC screening score",False)]
    fig, axes = plt.subplots(1, len(curve_specs), figsize=(18, 11), sharey=True)
    for ax,(key,title,is_log) in zip(axes,curve_specs):
        values=[float(r[key]) if r.get(key) is not None else float("nan") for r in plot_samples]
    ...
    axes[0].invert_yaxis()
    fig.suptitle(f"PetroEdge Multi-track Log Interpretation - {dataset.get('name', dataset_id)}",fontsize=14,fontweight="bold")
    ...
    styles=getSampleStyleSheet();story=[Paragraph("PetroEdge AI - Log Signature Report",styles["Title"]),Spacer(1,8)]
    fluid=str(summary.get("primary_fluid","uncertain")).replace("_"," ").title();confidence=float(summary.get("fluid_confidence",0))*100
    table_data=[["Dataset",dataset.get("name",dataset_id),"Fluid screening",fluid],["Rows interpreted",summary.get("rows_interpreted"),"Fluid screening score",f"{confidence:.1f}%"],["Mean porosity",f"{float(summary.get('mean_porosity',0))*100:.1f}%","Mean Sw",f"{float(summary.get('mean_water_saturation',0))*100:.1f}%"],["Pay intervals",summary.get("pay_intervals"),"HC screening score",f"{float(summary.get('mean_hydrocarbon_probability',0))*100:.1f}%"]]
    ...
    story.extend([t,Spacer(1,12),Image(image_buffer,width=1100,height=580),Spacer(1,10),Paragraph("Interpretation note",styles["Heading2"]),Paragraph("These are uncalibrated empirical screening estimates, not trained probabilities. Sonic is used only where measured; missing inputs are not invented. Confirm oil or gas using pressure-gradient data, formation testing, PVT, mud-log shows or production evidence before operational decisions.",styles["BodyText"])])
...
@router.post("/datasets/{dataset_id}/measured-evidence")
def measured_evidence(dataset_id:str,payload:AssistantRequest,_:dict[str,Any]=Depends(READ_ROLES)):
    from app.services.measured_evidence import snapshot,answer
    try:
        if get_dataset_path(dataset_id).suffix.lower()==".pdf":
            from app.services.document_evidence import retrieve
            return retrieve(dataset_id,payload.question)
        return answer(snapshot(dataset_id),payload.question)
    except KeyError as exc:raise HTTPException(404,"Dataset not found") from exc
    except (ValueError,OSError) as exc:raise HTTPException(422,str(exc)) from exc
