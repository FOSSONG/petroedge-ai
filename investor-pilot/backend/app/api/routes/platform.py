...
def actor(user: dict[str, Any]) -> str | None:
    from app.core.ownership import identity
...
def overview(_: dict[str, Any] = Depends(READ_ROLES)) -> PlatformOverview:
    ...
    accessible_experiments = list_experiments()
    experiments = len(accessible_experiments)
    completed = sum(item.status == "completed" for item in accessible_experiments)
...
def dataset_preview(dataset_id: str, offset: int = 0, limit: int = 100, _: dict[str, Any] = Depends(READ_ROLES)) -> DatasetPreview:
    try:
        return DatasetPreview(**preview_dataset(dataset_id, max(0, offset), max(1, min(limit, 250))))
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Dataset not found.") from exc
