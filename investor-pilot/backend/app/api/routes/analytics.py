...
@router.get("/saved")
def saved_analyses(_: dict[str, Any] = Depends(require_roles("viewer"))):
    from app.services.analysis_records import list_records
    return list_records()


@router.get("/saved/{analysis_id}")
def saved_analysis(analysis_id: str, _: dict[str, Any] = Depends(require_roles("viewer"))):
...
class SavedEvidenceQuestion(BaseModel):
    model_config = ConfigDict(extra="forbid")
    question: str = Field(min_length=3, max_length=1000)

@router.post("/saved/{analysis_id}/assistant")
def saved_evidence(analysis_id: str, payload: SavedEvidenceQuestion,
                   _: dict[str, Any] = Depends(require_roles("viewer"))):
    from app.services.analysis_evidence import answer
    return answer(analysis_id, payload.question)
