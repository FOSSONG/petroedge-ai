...
ROUTE_MODULES: tuple[tuple[str, str, tuple[str, ...], bool], ...] = (
    ("pilot", "/pilot", ("Pilot owner controls",), True),
    ...
    ("rig_streaming", "/rig-streaming", ("Rig Streaming",), True),
    ("stream", "", ("Stream",), False),
    ("upload", "", ("Upload",), False),
)
...
async def lifespan(application: FastAPI):
    if settings.environment.lower()=="production":
        if not settings.pilot_owner_email:
            raise RuntimeError("Production pilot requires a configured sole owner.")
        if settings.phone_approval_required:
            from app.services.phone_approval import client as phone_client
            phone_client()  # Optional protection remains fail-closed when enabled.
...
def create_app() -> FastAPI:
    ...
    application.add_middleware(PilotBoundaryMiddleware)
    ...
    application.add_middleware(OwnershipContextMiddleware)
