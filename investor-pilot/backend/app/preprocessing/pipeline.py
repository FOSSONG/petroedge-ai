...
def _interpolate_short_gaps(
    ...
    *,
    ...
    for curve in curves:
        ...
        # Never partially bridge a longer gap from each end.
        missing = series.isna()
        runs = missing.ne(missing.shift()).cumsum()
        run_size = missing.groupby(runs).transform("sum")
        filled = filled.where(~missing | run_size.le(max(0, int(limit))), series)
