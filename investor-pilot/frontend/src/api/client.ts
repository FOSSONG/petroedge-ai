...
export async function login(
  ...
  const response = await apiRequest<LoginResponse & {mfa_required?:boolean;authorization_url?:string}>("/auth/login", {
    credentials: "include",
    method: "POST",
  ...
  if (!response.access_token) {
    if(response.mfa_required&&response.authorization_url){window.location.assign(response.authorization_url);throw new Error("Continue with phone approval. No access has been granted yet.");}
    throw new Error(
