...
const definitions=[
 ...
 {title:"DT vs RHOB",x:"sonic_usft",y:"density_gcc",xlabel:"DT (us/ft)",ylabel:"RHOB (g/cm3)",log:false},
 {title:"DT vs NPHI",x:"sonic_usft",y:"neutron_porosity_vv",xlabel:"DT (us/ft)",ylabel:"NPHI (v/v)",log:false},
 {title:"GR vs DT",x:"gamma_ray_api",y:"sonic_usft",xlabel:"GR (API)",ylabel:"DT (us/ft)",log:false},
 {title:"DT vs RT",x:"sonic_usft",y:"resistivity_ohmm",xlabel:"DT (us/ft)",ylabel:"RT (ohm.m)",log:false,logY:true},
 {title:"Caliper vs RHOB",x:"caliper_in",y:"density_gcc",xlabel:"Caliper (in)",ylabel:"RHOB (g/cm3)",log:false},
 {title:"Caliper vs NPHI",x:"caliper_in",y:"neutron_porosity_vv",xlabel:"Caliper (in)",ylabel:"NPHI (v/v)",log:false},
...
export function WellLogViewer(){
 ...
 const charts=useMemo(()=>(<Box display="grid" gridTemplateColumns={{xs:"1fr",xl:"repeat(2,1fr)"}} gap={2}>{definitions.filter(def=>def.title===activePlot).map(def=>{
   ...
   const interpretation=interpretPairs(valid,def.x,def.y,Boolean(def.log),Boolean(def.log||def.logY));
   ...
   if(interpretation.highlight.length)traces.push({type:"scatter",mode:"markers",name:"Points of interest",x:interpretation.highlight.map(r=>r[def.x]),y:interpretation.highlight.map(r=>r[def.y]),customdata:interpretation.highlight.map(r=>r.depth_m),marker:{size:15,symbol:"circle-open",color:"#e56b00",line:{width:2}},hovertemplate:"Circled sample: %{x}, %{y}<br>MD %{customdata} m<extra></extra>"});
 ...
 return <Stack spacing={3}>
  ...
  <Stack direction="row" flexWrap="wrap" gap={1}>{definitions.map(def=><Button key={def.title} variant={activePlot===def.title?"contained":"outlined"} onClick={()=>setActivePlot(def.title)}>View {def.title}</Button>)}</Stack>
  {!activePlot&&<Typography>Select a plot above. Only the selected chart is loaded.</Typography>}
  {charts}
