      ...
      >
        ...
        >
          <Stack spacing={3}>
            ...
            <Alert severity="info">Sign in with your PetroEdge account. Contact the platform owner if you need access.</Alert>
            {new URLSearchParams(window.location.search).get("phone_approved")==="1"&&<Button disabled={loading} onClick={async()=>{setLoading(true);try{const auth=await apiRequest<LoginResponse>("/auth/duo/complete",{method:"POST",credentials:"include"});persistSession(auth);window.history.replaceState({},"",window.location.pathname);onAuthenticated(fallbackUser(auth,""));}catch(e){setError(authenticationMessage(e));}finally{setLoading(false)}}}>Complete phone-approved sign-in</Button>}
