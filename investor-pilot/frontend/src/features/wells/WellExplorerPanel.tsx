import { RegisterMappedWell } from "./RegisterMappedWell";
import { WellDatasetBindingPanel } from "./WellDatasetBindingPanel";
import {
  useMemo,
  useState,
} from "react";
...
export function WellExplorerPanel() {
  ...
  const analysisMutation =
    useMutation({
      mutationFn: ({
        ...
        analyseBoundRow(wellId, Number(asRecord(sample).source_row), String(asRecord(sample).binding_sha256 ?? "")),
    });
  ...
  return (
    <Stack spacing={3}>
      ...
      {activeWellId && <WellDatasetBindingPanel key={activeWellId} wellId={activeWellId} />}
      ...
      >
        ...
        >
          <Stack spacing={2}>
            ...
            {analysisMutation.data && asRecord(analysisMutation.data).provenance != null && <Paper variant="outlined" sx={{ p: 2 }}><Typography variant="h6">Source provenance</Typography><Typography component="pre" sx={{ whiteSpace: "pre-wrap", fontSize: 12 }}>{JSON.stringify(asRecord(analysisMutation.data).provenance, null, 2)}</Typography></Paper>}
