...
export function AiAgentsPanel(){
 ...
 return <Stack spacing={3}>
  ...
  <Paper variant="outlined" sx={{p:3}}><Stack spacing={2}><Stack direction={{xs:"column",md:"row"}} gap={2}><FormControl fullWidth><InputLabel>Agent</InputLabel><Select value={activeAgent} label="Agent" onChange={e=>{setAgentKey(e.target.value);single.reset();panel.reset()}}>{(agents.data??[]).map(a=><MenuItem key={a.key} value={a.key}>{a.name}</MenuItem>)}</Select></FormControl><FormControl fullWidth><InputLabel>Dataset</InputLabel><Select value={activeDataset} label="Dataset" onChange={e=>{setDatasetId(e.target.value);single.reset();panel.reset()}}>{(datasets.data??[]).map(d=><MenuItem key={d.dataset_id} value={d.dataset_id}>{d.name}</MenuItem>)}</Select></FormControl></Stack><TextField multiline minRows={2} label="Analysis objective" value={objective} onChange={e=>{setObjective(e.target.value);single.reset();panel.reset()}}/><Stack direction="row" gap={1}><Button variant="contained" startIcon={<Play/>} disabled={!activeAgent||single.isPending||panel.isPending} onClick={()=>{panel.reset();single.mutate()}}>Review specialist evidence</Button><Button variant="outlined" startIcon={<Bot/>} disabled={single.isPending||panel.isPending} onClick={()=>{single.reset();panel.mutate()}}>Review panel evidence</Button></Stack></Stack></Paper>
  ...
  {result&&<Paper variant="outlined" sx={{p:3}}><Typography variant="h6">Panel guidance</Typography><Typography mt={1}>{result.consensus}</Typography><Stack direction="row" gap={1} mt={2}><Chip label="Confidence not calculated"/><Chip label="Agreement not calculated"/></Stack>{result.recommendations.map(x=><Typography key={x} mt={1}>• {x}</Typography>)}</Paper>}
