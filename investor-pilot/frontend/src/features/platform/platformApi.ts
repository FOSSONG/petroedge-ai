...
export const fetchDatasetPreview = (datasetId: string, offset = 0, limit = 100) =>
  apiRequest<DatasetPreview>(`/platform/datasets/${datasetId}/preview?offset=${offset}&limit=${limit}`);
...
export async function downloadReport(reportId: string, format = "pdf") {
  ...
  if (format === "pdf" && !blob.type.toLowerCase().includes("application/pdf")) throw new Error("The server did not return a PDF report.");
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a"); anchor.href = url; anchor.download = `petroedge-report-${reportId}.${format}`; document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(url),300000);
...
export interface LasWizardResult { dataset_id:string; detected_curves:string[]; mnemonic_mapping:Record<string,string|null>; source_units?:Record<string,string>; match_details?:Record<string,{status:string;method:string;candidates:string[]}>; blocking_issues?:string[]; qc:{row_count:number;missing_cells:number;duplicate_rows:number;ready:boolean}; missing_recommended_curves:string[]; steps:string[]; }
...
export interface PreparedTrainingDataset {
  dataset_id: string; name: string; rows: number; target: string; condition: string;
  status: string; reason: string; feature_columns: string[];
}
export const fetchPreparedTrainingDatasets = () =>
  apiRequest<{datasets: PreparedTrainingDataset[]}>("/training-lifecycle/prepared-datasets");

export interface IndependentEvaluationResult {
  model_id: string; passed: boolean; metrics: Record<string, number>; failures: string[];
  test_rows: number; test_groups: number; evaluated_at: string; actor: string;
}
export interface IndependentEvaluationStatus {
  status: string;
  policy?: {policy_id: string; minimum_test_rows: number; minimum_test_groups: number; criteria: Record<string, {min?:number;max?:number}>};
  result?: IndependentEvaluationResult;
}
export const fetchIndependentEvaluation = (id:string) => apiRequest<IndependentEvaluationStatus>(`/training-lifecycle/registry/${encodeURIComponent(id)}/independent-evaluation`);
export const runIndependentEvaluation = (id:string) => apiRequest<IndependentEvaluationResult>(`/training-lifecycle/registry/${encodeURIComponent(id)}/independent-evaluation`, {method:"POST"});


export interface PredictionCompatibility {
 ready:boolean;required_features:string[];missing_columns:string[];empty_columns:string[];
 invalid_counts:Record<string,number>;total_rows:number;complete_rows:number;messages:string[];policy:string;
}
export const fetchPredictionCompatibility=(modelId:string,datasetId:string)=>apiRequest<PredictionCompatibility>("/training-lifecycle/prediction-compatibility",{method:"POST",body:JSON.stringify({model_id:modelId,dataset_id:datasetId})});
