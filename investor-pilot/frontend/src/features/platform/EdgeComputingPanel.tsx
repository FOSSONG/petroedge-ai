...
type RigConnectorStatus = { state:string; connector_type:string; endpoint?:string|null; last_message_at?:string|null; last_delivery_at?:string|null; last_error?:string|null; messages_received:number; messages_delivered:number; reconnect_count:number; buffer:{depth:number;capacity:number;oldest_received_at?:string|null;failed_delivery_attempts:number}; etp_conformance:string; transport_note:string; };
...
const fetchRigConnectorStatus=()=>apiRequest<RigConnectorStatus>("/rig-streaming/status");
...
export function EdgeComputingPanel(){
  ...
  const rigConnector=useQuery({queryKey:["edge","rig-connector"],queryFn:fetchRigConnectorStatus,refetchInterval:5000});
  ...
  return <Stack spacing={3}>
    <PilotBenchmarkPanel kind="edge"/>
    <Paper className="hero-panel" sx={{p:3}}><Stack direction={{xs:"column",md:"row"}} justifyContent="space-between" gap={2}><Stack direction="row" gap={1.5} alignItems="center"><Cpu/><Box><Typography variant="h4" fontWeight={900}>Edge Computing</Typography><Typography>Depth-indexed real-time petrophysical interpretation, edge replay and offline synchronisation.</Typography></Box></Stack><Stack direction="row" gap={1}><Chip color="success" label={`Release ${capabilities.data?.release??"2.2.0"}`}/><Button startIcon={<RefreshCw size={17}/>} onClick={()=>void queryClient.refetchQueries({type:"active"})}>Refresh</Button></Stack></Stack></Paper>
    <Paper variant="outlined" sx={{p:2.5}}>
      <Stack spacing={1.5}>
        <Stack direction={{xs:"column",md:"row"}} justifyContent="space-between" gap={1}>
          <Box>
            <Typography variant="h6" fontWeight={900}>B3 rig streaming transport</Typography>
            <Typography variant="body2" color="text.secondary">Durable store-and-forward transport between a rig WITSML/ETP gateway and the B1/B2 interpretation pipeline.</Typography>
          </Box>
          <Stack direction="row" gap={1} flexWrap="wrap">
            <Chip label={rigConnector.data?.state ?? "UNKNOWN"} color={rigConnector.data?.state==="ONLINE"?"success":rigConnector.data?.state==="ERROR"?"error":"warning"}/>
            <Chip label={`Buffer ${rigConnector.data?.buffer.depth ?? 0}`} />
            <Chip label={`Reconnects ${rigConnector.data?.reconnect_count ?? 0}`} />
          </Stack>
        </Stack>
        {rigConnector.isLoading&&<LinearProgress/>}
        {rigConnector.isError&&<Alert severity="warning">Rig connector status is unavailable.</Alert>}
        {rigConnector.data&&<Grid container spacing={1.5}>
          <Grid item xs={12} md={4}><Paper variant="outlined" sx={{p:1.5}}><Typography variant="caption" color="text.secondary">Delivered / received</Typography><Typography variant="h6">{rigConnector.data.messages_delivered} / {rigConnector.data.messages_received}</Typography></Paper></Grid>
          <Grid item xs={12} md={4}><Paper variant="outlined" sx={{p:1.5}}><Typography variant="caption" color="text.secondary">Durable buffer</Typography><Typography variant="h6">{rigConnector.data.buffer.depth} / {rigConnector.data.buffer.capacity}</Typography></Paper></Grid>
          <Grid item xs={12} md={4}><Paper variant="outlined" sx={{p:1.5}}><Typography variant="caption" color="text.secondary">ETP conformance</Typography><Typography variant="h6">{rigConnector.data.etp_conformance}</Typography></Paper></Grid>
        </Grid>}
        {rigConnector.data?.last_error&&<Alert severity="warning">{rigConnector.data.last_error}</Alert>}
        <Alert severity="info">B3 does not claim native Energistics ETP 1.2 certification. Its WebSocket mode consumes PetroEdge canonical JSON from an ETP/WITSML gateway. Native ETP protocol negotiation will require validation against the target rig server.</Alert>
      </Stack>
    </Paper>
