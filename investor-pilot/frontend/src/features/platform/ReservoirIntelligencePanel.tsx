...
const number = (value: unknown): number | null => { if (value === null || value === undefined || value === "") return null; const parsed = Number(value); return Number.isFinite(parsed) ? parsed : null; };
const percentage = (value: unknown) => number(value) === null ? "Unavailable" : `${(number(value)! * 100).toFixed(1)}%`;
...
export function ReservoirIntelligencePanel() {
  ...
  const [refreshState, setRefreshState] = useState({version:0,message:"",error:false});
  ...
  const chart = useMemo(() => {
    ...
    const samples = raw.filter((sample) => number(sample.depth) !== null);
    const plotSamples: typeof samples = [];
    samples.forEach((sample, index) => {
      if (index && Number(sample.source_row) !== Number(samples[index - 1].source_row) + 1) {
        plotSamples.push({ depth: sample.depth });
      }
      plotSamples.push(sample);
    });
    const depth = plotSamples.map((sample) => number(sample.depth));
    const trace = (field: string, name: string, axis: string) => ({
      x: plotSamples.map((sample) => number((sample as Record<string, unknown>)[field])), y: depth,
    ...
    const probability = (fluid: string, name: string) => ({
      x: plotSamples.map((sample) => number((sample as Record<string, any>).fluid_probabilities?.[fluid]) !== null ? (number((sample as Record<string, any>).fluid_probabilities?.[fluid]) as number) * 100 : null),
    ...
    return {
      ...
      data: [
        ...
        trace("porosity", "Porosity", "x5"), trace("water_saturation", "Sw", "x6"), trace("hydrocarbon_probability", "HC screening score", "x7"),
      ...
      layout: {
        ...
        xaxis7: axis("HC", domains[6], { range: [0,1] }), xaxis8: axis("Fluid score (%)", domains[7], { range: [0,100] }),
      },
    };
  }, [interpretation.data]);
  ...
  const refresh = async () => {
    const results = await Promise.all([datasets.refetch(), interpretation.refetch()]);
    const failed = results.find(result => result.isError);
    setRefreshState(previous => ({version:previous.version+1, error:Boolean(failed), message:failed ? (failed.error instanceof Error ? failed.error.message : "Refresh failed") : "Feature refreshed. Measurements and plots reloaded; chart zoom reset."}));
  };

  return <Stack spacing={3}>
    {refreshState.message&&<Alert severity={refreshState.error?"error":"success"}>{refreshState.message}</Alert>}
    <Paper className="hero-panel" sx={{ p: { xs: 3, md: 4 } }}>
      <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" gap={2}>
        ...
        <Stack minWidth={{ md: 350 }} gap={1.25}>
          ...
          <Stack direction="row" gap={1}><Button fullWidth variant="contained" startIcon={<RefreshCw size={16}/>} disabled={!datasetId || interpretation.isFetching} onClick={refresh}>Refresh</Button><PdfDownloadButton key={datasetId} datasetId={datasetId}/></Stack>
        </Stack>
      </Stack>
    </Paper>

    <SaturationScenarioPanel/>
    ...
    {interpretation.data && <>
      <Alert severity="info">Optional curves unavailable: {String(summary.missing_optional_curves || "none")}. Rows screened: {String(summary.rows_interpreted)} / {String(summary.source_rows)}. Excluded by QC: {String(summary.excluded_rows)}. {String(summary.calibration)}. Assumptions: {String(summary.assumptions)}</Alert>
      <Grid container spacing={2}>
        <Grid item xs={12} md={4}><Paper variant="outlined" sx={{ p: 2.5, height: "100%" }}><Stack direction="row" gap={1} alignItems="center"><Droplets size={20}/><Typography variant="h6">Predominant fluid</Typography></Stack><Typography variant="h4" mt={2}>{label(summary.primary_fluid)}</Typography><Typography color="text.secondary">Normalised screening score: {percentage(summary.fluid_confidence)}</Typography><Chip sx={{ mt: 2 }} label={label(summary.fluid_method ?? "log screening")} /></Paper></Grid>
        {(["oil","gas","water"] as const).map((fluid) => <Grid item xs={12} sm={4} md={8/3} key={fluid}><Paper variant="outlined" sx={{ p: 2.5, height: "100%" }}><Typography color="text.secondary">{label(fluid)} screening score</Typography><Typography variant="h4" mt={1}>{percentage(fluidProbabilities[fluid])}</Typography></Paper></Grid>)}
      </Grid>

      <Alert severity="warning">These are normalised empirical screening scores, not calibrated oil/gas/water probabilities. Porosity, saturation and permeability are empirical estimates. Confirm oil or gas using pressure gradients, formation testing, mud-gas, PVT, fluid sampling or production evidence.</Alert>

      {chart.samples.length > 0 ? <Paper variant="outlined" sx={{ p: 1.5, overflow: "hidden" }}><SafePlot key={refreshState.version} data={chart.data} layout={chart.layout} minHeight={760}/></Paper> : <Alert severity="warning">No valid depth-linked samples were returned for plotting.</Alert>}
