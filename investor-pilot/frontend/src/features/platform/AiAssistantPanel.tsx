import {MeasuredEvidencePanel} from "./MeasuredEvidencePanel";
...
export function AiAssistantPanel(){
 ...
 return <Stack spacing={3}>
  <Button disabled={ds.isFetching||m.isPending} onClick={()=>{m.reset();void ds.refetch()}}>Refresh assistant datasets</Button>
  <Paper variant="outlined" sx={{p:3}}><Typography variant="h5">Saved-analysis evidence</Typography><Typography>Answers below retrieve stored results with source references; they do not run new inference.</Typography><SavedAnalysesPanel /></Paper>
  <Alert severity="info">The dataset assistant below is a separate rule-based interpretation workflow, not a lookup of saved analyses.</Alert>
  ...
  <MeasuredEvidencePanel key={id} id={id}/>
