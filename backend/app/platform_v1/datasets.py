...
def preview_dataset(dataset_id: str, offset: int = 0, limit: int = 100):
    ...
    if suffix == ".csv":
        ...
        frame = pd.read_excel(path, skiprows=range(1, offset + 1), nrows=limit)
    elif suffix == ".las":
        import lasio
        frame = lasio.read(path).df().reset_index().iloc[offset : offset + limit]
    else:
        raise ValueError("Unsupported dataset type.")
...
def _register_derived_frame(parent_id: str, frame, name: str, operation: dict[str, Any], actor_id: str | None):
    ...
    with connection() as conn:
        ...
        conn.execute("INSERT INTO dataset_lineage(parent_dataset_id,child_dataset_id,operation,details_json,created_at) VALUES (?,?,?,?,?)",(parent_id,dataset_id,operation.get("operation","derived"),json.dumps(operation),now))
...
def prepare_dataset(dataset_id: str, request, actor_id: str | None):
    ...
    name=request.name or f"{get_dataset(dataset_id).name} · ML-ready"
