  ...
  return <Stack spacing={3}>
    ...
    {tab===2&&<Stack spacing={1.5}>
      {(models.data?.models??[]).map(m=><Paper key={m.model_id} variant="outlined" sx={{p:2.5}}>
        <Stack direction={{xs:"column",md:"row"}} justifyContent="space-between" gap={2}>
          <Box><Typography fontWeight={800}>{m.display_name} · v{m.version}</Typography><Typography variant="body2" color="text.secondary">{pretty(m.algorithm)} · {pretty(m.task_type)} · {pretty(m.validation_strategy)}</Typography><Typography variant="caption" color="text.secondary" display="block">Created: {new Date(m.created_at).toLocaleString()}{m.validated_at?` · Validated: ${new Date(m.validated_at).toLocaleString()}`:""}{m.deployed_at?` · Deployed: ${new Date(m.deployed_at).toLocaleString()}`:""}{m.retrained_at?` · Retrained: ${new Date(m.retrained_at).toLocaleString()}`:""}</Typography><Stack direction="row" gap={1} mt={1} flexWrap="wrap"><Chip size="small" label={m.stage} color={m.stage==="production"?"success":m.stage==="validated"?"primary":"default"}/>{Object.entries(m.metrics).map(([k,v])=><Chip key={k} size="small" variant="outlined" label={`${pretty(k)}: ${Number(v).toFixed(4)}`}/>)}</Stack></Box>
    ...
    <Dialog open={Boolean(retrainTarget)} onClose={()=>!retrain.isPending&&setRetrainTarget(null)} maxWidth="sm" fullWidth>
        ...
        <Stack spacing={2} mt={1}>
          ...
          <FormControl fullWidth>
            ...
            <Select label="New training dataset" value={retrainDatasetId} onChange={event=>setRetrainDatasetId(event.target.value)}>
              {(datasets.data??[]).map(dataset=><MenuItem key={dataset.dataset_id} value={dataset.dataset_id}>{dataset.name} · {dataset.row_count??0} rows</MenuItem>)}
            </Select>
          </FormControl>
      ...
      <DialogActions>
        ...
        <Button variant="contained" startIcon={<RefreshCcw size={16}/>} disabled={!retrainTarget||!retrainDatasetId||retrain.isPending} onClick={()=>retrain.mutate()}>{retrain.isPending?"Retraining…":"Retrain model"}</Button>
      </DialogActions>
    </Dialog>
    ...
    <Dialog open={Boolean(deleteTarget)} onClose={()=>!remove.isPending&&setDeleteTarget(null)} maxWidth="sm" fullWidth>
      ...
      <DialogActions>
        ...
        <Button color="error" variant="outlined" sx={{opacity:.72}} startIcon={<Trash2 size={16}/>} disabled={!deleteTarget||remove.isPending} onClick={()=>deleteTarget&&remove.mutate(deleteTarget.model_id)}>{remove.isPending?"Deleting…":"Delete model"}</Button>
      </DialogActions>
    </Dialog>
  </Stack>
}
