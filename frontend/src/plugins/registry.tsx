...
const makePanel = (title: string, description: string, resourceClass: "light"|"medium"|"heavy", capabilities: string[]) => lazy(async () => {
  const { CapabilityModulePanel } = await import("../features/plugins/CapabilityModulePanel");
  return { default: () => <CapabilityModulePanel title={title} description={description} resourceClass={resourceClass} capabilities={capabilities}/> };
});
...
export const builtInModules: ModuleDefinition[] = specs.map(([key,name,description,resourceClass,capabilities,icon]) => ({ key,name,description,resourceClass,capabilities:[...capabilities],icon,component:makePanel(name,description,resourceClass,[...capabilities]) }));
